...
class TournamentServiceTest {
    ...
    @Nested
    @DisplayName("Player Selection Tests")
    class PlayerSelectionTests {

        @Test
        @DisplayName("Should throw when non-admin tries to select player")
        void selectPlayer_ShouldThrow_WhenNotAdmin() {
            // Arrange
            when(userSessionRepository.findBySessionId("test-session")).thenReturn(userSession);
        ...
        @Test
        @DisplayName("Admin should be able to select enrolled player")
        void selectPlayer_ShouldSucceed_WhenAdminAndPlayerEnrolled() {
            // Arrange
            user.setRole(adminRole);
            userSession.setUser(user);
            tournamentEnrollment.setStatus(EnrollmentStatus.PENDING);
            ...
            verify(tournamentEnrollmentRepository).save(tournamentEnrollment);
        }

        @Test
        @DisplayName("Should throw when selecting more than maximum players")
        void selectPlayer_ShouldThrow_WhenExceedingMaxPlayers() {
            // Arrange
            user.setRole(adminRole);
            userSession.setUser(user);
